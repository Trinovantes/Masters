...
int main(int argc, char **argv)
{
  ...
  do {
    ...
    if (time_loop_end == 0) 
      time_loop_end = time(NULL) - time_loop_start;
  } while (!received_all_fragments);

  int time_write_start = time(NULL);
  ...
  printf("elapsed time: %ld", time(NULL) - start);
  printf("elapsed time sequential: %ld", (time(NULL) - time_write_start) + time_loop_end);
